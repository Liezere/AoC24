...
draw_map(Map, Robot) :-
    forall(draw_tile(Map, Robot), true),
    nl,
...
draw_tile(Map, Robot) :-
    map_width(W),
    between(0, W, Y),
    ( map_at(Map, 0-Y, T0), ground(T0), T0 = wall -> nl ),
    between(0, W, X),
    (
        Robot = X-Y
    ->  format_tile(robot)
    ;   map_at(Map, X-Y, T), ground(T),
        format_tile(T)
    ).

format_tile(robot) :- put_char('@').
format_tile(empty) :- put_char('.').
format_tile(wall) :- put_char('#').
format_tile(stone(o)) :- put_char('O').
format_tile(stone(right)) :- put_char('[').
format_tile(stone(left)) :- put_char(']').
